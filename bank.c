...
void get_transaction_history(int account_number, char *history, size_t history_size, int entered_pin, int *pin_ok) {
    *pin_ok = 0;
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        double amt, bal;
        if (sscanf(line, "%d;%31[^;];%15[^;];%lf;%lf", &anum, tstamp, typ, &amt, &bal) == 5) {
...
void get_transaction_history_admin(int account_number, char *history, size_t history_size) {
    ...
    fclose(fp);
    if (!found) strncat(history, "No transactions made.\n", history_size - strlen(history) - 1);
}
