...
GtkWidget *admin_dialog = NULL, *admin_search_entry = NULL, *admin_details_box = NULL;
...
int main(int argc, char *argv[]) {
    ...
    gtk_window_set_default_size(GTK_WINDOW(window), 390, 600);
...
static void show_create_account(GtkWidget *widget, gpointer data) {
    ...
    GtkWidget *btn_submit = gtk_button_new_with_label("Create");
...
static void on_create_account_submit(GtkWidget *widget, gpointer data) {
    ...
    if (strlen(uname) == 0) {
        MSGBOX(data, GTK_MESSAGE_ERROR, "Please enter a name."); return;
    }
    int accnum, pin;
    if (!create_account_gui(uname, &accnum, &pin)) {
        MSGBOX(data, GTK_MESSAGE_ERROR, "File error."); return;
    }
    char msg[256];
...
static void show_view_balance(GtkWidget *widget, gpointer data) {
    ...
    GtkWidget *label1 = gtk_label_new("Account number:"), *label2 = gtk_label_new("PIN:");
    ...
    GtkWidget *btn_submit = gtk_button_new_with_label("View");
...
static void on_view_balance_submit(GtkWidget *widget, gpointer data) {
    ...
    if (acc) {
        char msg[256]; snprintf(msg, sizeof(msg), "Welcome, %s!\n\nAccount: %d\nBalance: Rs. %.2lf", acc->name, acc->account_number, acc->balance); MSGBOX(data, GTK_MESSAGE_INFO, msg);
        gtk_widget_destroy(GTK_WIDGET(data));
    } else {
...
static void show_deposit(GtkWidget *widget, gpointer data) {
    ...
    gtk_window_set_default_size(GTK_WINDOW(dialog), 320, 170);
    GtkWidget *grid = gtk_grid_new(); gtk_container_add(GTK_CONTAINER(dialog), grid);
...
static void on_deposit_submit(GtkWidget *widget, gpointer data) {
    ...
    if (deposit_fast(accnum, pin, amt, &balance)) {
        char msg[128]; snprintf(msg, sizeof(msg), "Deposit successful.\nNew balance: Rs. %.2lf", balance); MSGBOX(data, GTK_MESSAGE_INFO, msg);
        gtk_widget_destroy(GTK_WIDGET(data));
    } else {
...
static void on_withdraw_submit(GtkWidget *widget, gpointer data) {
    ...
    if (withdraw_fast(accnum, pin, amt, &balance)) {
        char msg[128]; snprintf(msg, sizeof(msg), "Withdraw successful.\nNew balance: Rs. %.2lf", balance); MSGBOX(data, GTK_MESSAGE_INFO, msg); gtk_widget_destroy(GTK_WIDGET(data));
    } else {
...
static void show_admin_details(GtkWidget *widget, gpointer data) {
    ...
    gtk_window_set_default_size(GTK_WINDOW(dialog), 650, 380);
    GtkWidget *grid = gtk_grid_new();
    gtk_container_add(GTK_CONTAINER(dialog), grid);
